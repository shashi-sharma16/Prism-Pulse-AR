...
import { VisualsEngine } from './visuals.js';
import { PoseAnalyzer, getDist } from './poses.js';
...
// Config bound to UI sliders
let activeCamera = 'user';
let bgMode = 'matrix';
let particleDensity = 0.55;
let arcSensitivity = 160;
let trailLength = 0.35;
...
const visuals = new VisualsEngine();
const poses = new PoseAnalyzer();

/**
 * Theme color functions
 */
const themePalette = {
    'Cyberpunk': (t, val, max) => (val % 2 === 0) ? '#ff0055' : '#00e5ff',
    'Solar': (t, val, max) => `hsl(${40 + Math.sin(t * 2 + val) * 15}, 100%, ${55 + Math.sin(t * 3) * 10}%)`,
    'Ocean': (t, val, max) => `hsl(${192 + (val * 12)}, 90%, 55%)`,
    'Lava': (t, val, max) => `hsl(${(10 + (val * 10)) % 40}, 100%, ${48 + Math.sin(t * 3) * 10}%)`,
    'Nebula': (t, val, max) => `hsl(${270 + Math.sin(t * 1.5 + val) * 40}, 100%, 65%)`
};

function getThemeColor(val, max) {
    return themePalette[currentTheme](time, val, max);
}
...
/**
 * Maps normalized MediaPipe landmarks to canvas pixel space.
 */
...
lightningSlider.addEventListener('input', (e) => {
    arcSensitivity = parseInt(e.target.value, 10);
    lightVal.innerText = e.target.value;
});

blurSlider.addEventListener('input', (e) => {
    trailLength = parseFloat(e.target.value) / 100;
...
/**
 * Camera setup via MediaPipe's Camera helper
 */
async function initCamera(facingModeValue) {
    ...
    try {
        ...
        alert("Unable to access the camera. Check your browser's permission settings.");
    }
}
    ...
    handsInstance.onResults((results) => {
        ...
        if (currentHands.length > 0 && results.multiHandLandmarks && results.multiHandLandmarks.length > 0) {
            ...
            if (oldIndexPt && newIndexPt) {
                handSpeed = getDist(oldIndexPt, newIndexPt);
            }
        } else {
            handSpeed = 0;
        }
        ...
        sound.updateHum(currentHands);
    });
}

/**
 * Start button kicks off the whole experience
 */
startBtn.addEventListener('click', async () => {
    ...
    sound.init();
    sound.setVolume(parseFloat(volumeSlider.value) / 100);
...
function renderLoop(timestamp) {
    ...
    visuals.drawBackground(bgCtx, width, height, getThemeColor, time, handSpeed, bgMode, trailLength);

    // 2. Fade + prep main overlay canvas
    ...
    // 3. Particles & ripples
    visuals.updatePhysics(ctx);

    // 4. Hand skeleton + fingertip glow
    if (currentHands.length > 0) {
        currentHands.forEach((hand, handIndex) => {
            ...
            FINGER_TIPS.forEach((tipIndex, idx) => {
                ...
                if (Math.random() > 0.5) {
                    visuals.createParticles(pt, tipColor, particleDensity);
                }
            });
            ctx.shadowBlur = 0;
        });

        // 5. Two-hand light bridge + mandala geometry
        if (currentHands.length >= 2) {
            ...
            FINGER_TIPS.forEach((tipIndex, idx) => {
                ...
                if (dist < arcSensitivity && Math.random() > 0.45) {
                    ctx.beginPath();
        ...
        // 6. Pose telemetry + sound/ripple triggers
        const telemetry = poses.detect(currentHands, (midpoint) => {
            visuals.createRipple(mapToCanvas(midpoint), getThemeColor(2, 5));
            sound.triggerPulse();
        });

        uiGesture.innerText = telemetry.poseName;
        uiSpread.innerText = `${telemetry.opennessPct}%`;
    }
